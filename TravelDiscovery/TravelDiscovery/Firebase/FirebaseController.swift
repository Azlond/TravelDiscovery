...
class FirebaseController {
    ...
    public static func saveToFirebase() {
        if let user = Auth.auth().currentUser {
            FirebaseData.ref.child("users").child(user.uid).setValue(["visitedCountries": FirebaseData.visitedCountries])
        }
    }
    ...
    public static func retrieveFromFirebase() {
        if let user = Auth.auth().currentUser {
            ...
            FirebaseData.ref.child("users").child(user.uid).observeSingleEvent(of: .value, with: { (snapshot) in
                ...
                FirebaseData.visitedCountries = value?["visitedCountries"] as? Dictionary<String, Bool> ?? [:]
